...
if [ -z "$STATISTICS_PORT" ]; then
  STATISTICS_PORT=2398
fi
...
echo "$SECRET" | tr ',' '\n' | while read S; do
  echo "[*]   Secret $I: $S"
  echo "[*]   tg:// link for secret $I auto configuration: tg://proxy?server=${EXTERNAL_IP}&port=${HTTP_PORT}&secret=${S}"
  echo "[*]   t.me link for secret $I: https://t.me/proxy?server=${EXTERNAL_IP}&port=${HTTP_PORT}&secret=${S}"
  I=$(($I+1))
done
...
exec /mtproxy/mtproto-proxy "$@" --port  ${STATISTICS_PORT} --http-ports ${HTTP_PORT}  --aes-pwd ${REMOTE_SECRET} --user root ${REMOTE_CONFIG} --nat-info "$INTERNAL_IP:$EXTERNAL_IP" ${SECRET_CMD} ${TAG_CMD}